...
class ParentFirebaseFunctions {
  ...
  //   Get.log(
  //     'Adding new child with parent ID: ${FirebaseAuth.instance.currentUser!.uid} and normal loading value:${authController.isNormalLoading.value}',
  //   );
  //   if (addChildController.childName.value.isEmpty || addChildController.childAge.isEmpty) {
  ...
  //               title: "adding..",
  //             ));
  //     final String avatarUrl = addChildController.selectedAvatarPath.value.isEmpty && addChildController.kidImagePath.value.isEmpty
  ...
  //     DocumentReference newChildRef = _firebaseFirestore.collection('kids').doc();
  //     String kidDocumentId = newChildRef.id;
  //
  //     DocumentReference parentRef = _firebaseFirestore.collection('parents').doc(FirebaseAuth.instance.currentUser!.email);
  //
  //     String parentDocumentId = FirebaseAuth.instance.currentUser!.uid; // This is the parent ID
  ...
  //     authController.isNormalLoading.value = false;
  //     Get.log(
  //       'Added new child with parent ID: ${FirebaseAuth.instance.currentUser!.uid} and normal loading value:${authController.isNormalLoading.value}',
  //     );
  ...
  //     Get.log(
  //       'Error adding new child with parent ID: ${FirebaseAuth.instance.currentUser!.uid} and normal loading value:${authController.isNormalLoading.value}',
  //     );
  //     Get.log(e.toString());
  //   }
  // }
  ...
  //     );
  //
  //     DocumentReference kidDocRef = FirebaseFirestore.instance.collection('kids').doc(kidId);
  //
  //     DocumentSnapshot snapshot = await kidDocRef.get();
  //
  //     if (snapshot.exists) {
  //       // Check if 'spendings' exists, if not, initialize it with default values
  //       final currentSpending = (snapshot.data() as Map<String, dynamic>?)?['spendings']?['amount'] ?? 0.0; // Default to 0.0 if spendings don't exist
  ...
  //       String colorHex = '#${spendingJarColor.value.toRadixString(16).padLeft(8, '0').substring(2).toUpperCase()}'; // Format like "#RRGGBB"
  //
  //       // If the color is not already present, set the default color
  //       String currentColorHex = (snapshot.data() as Map<String, dynamic>?)?['spendings']?['color'] ?? '#227799';
  //
  ...
  //       final currentSpending = (data['spendings']?['amount'] ?? 0.0).toDouble();
  //

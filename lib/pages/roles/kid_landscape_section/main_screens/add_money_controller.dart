...
class AddMoneyController extends GetxController {
  // RxInt spendingAmount = 0.obs;
  // RxInt savingAmount = 0.obs;
  // RxInt totalValue = 0.obs;
  // var clickedIndex = 0.obs; // Observable for the text
  // RxBool isJarFilled = false.obs;
  // RxList<int> droppedNotes = <int>[].obs;
  // // var droppedNotes = <String>[].obs;
  ...
  //     final noteValue = int.parse(RegExp(r'\d+').stringMatch(noteValueString)!);

  //     droppedNotes.add(noteValue);

  //     // Update the total value
  //     totalValue.value = droppedNotes.fold(0, (sum, value) => sum + value);
  //   } catch (e) {
...
void onNoteDropped(String noteAsset) {
    try {
      // Extract numeric value from the asset name using regular expression
      final noteValueString = noteAsset.split('/').last.split('.').first;
      final noteValue = double.parse(RegExp(r'\d+').stringMatch(noteValueString)!);

      droppedNotes.add(noteValue);

      // Update the total value
      totalValue.value = droppedNotes.fold(0.0, (sum, value) => sum + value); // Changed to 0.0
    } catch (e) {
      print("Error parsing note asset: $e");
    }
  }
  ...
  // Method to fetch spending amount from Firebase
  ...
  //       print(
  //           'Fetched data: $data'); // Print the entire data to check what is being fetched

  //       final amount = data['spendings']?['amount'] ??
  //           0; // Use null-aware operator to avoid potential null errors
  //       spendingAmount.value = amount is int
  //           ? amount
  //           : (amount as double).toInt(); // Handle both int and double cases
  //       print(
  //           'Updated spendingAmount: ${spendingAmount.value}'); // Log the value for verification
  //     } else {
  //       print('No document found for the given parentId');
  //       spendingAmount.value = 0; // Default to 0 if no document is found
  //     }
  //   } catch (e) {
  //     print('Error fetching spending amount: $e');
  //     spendingAmount.value = 0; // Default to 0 in case of error
  //   }
  // }
  ...
  //             savingAmount.value = double.parse(sanitizedAmount).toInt();
  //           } else {
  //             print('Invalid amount format: "$sanitizedAmount"');
  //             savingAmount.value = 0;
  //           }
  //         } else if (amount is int) {
  //           savingAmount.value = amount;
  //         } else if (amount is double) {
  //           savingAmount.value = amount.toInt();
  //         } else {
...
Future<void> fetchSpendingAmount() async {
  ...
  // final Map<String, int> firstCoinList = {
  //   "assets/1euro.png": 1,
  //   "assets/2euros.png": 2,
  // };
  // final Map<String, int> secondCoinList = {
  //   "assets/50cent.png": 1,
  //   "assets/20cents.png": 2,
  //   "assets/10cents.png": 3,
  // };

        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.hasError ||
            ...
            return const Scaffold(
              ...
              body: Center(child: Text("Kid not found")),
            );
          }

          final data = snapshot.data!.data() as Map<String, dynamic>;
          final String kidName = data['name'] ?? 'Unnamed';

          return Scaffold(
            appBar: CustomAppBar(
              title: "$kidName's profile",
              centerTitle: true,
              showBackButton: true,
            ),
            body: SafeArea(
                                  ...
                                  return typeSwitcherContainer(
                                      containerColor:
                                          kidProfileManagementPageController
                                              ...
                                              : Color(0xFFEDFAFF),
                                      assetPath:
                                          'assets/kidManageIcons/coin.svg',
                                      onTap: () {
                                        kidProfileManagementPageController
  ...
  Widget jarData({required String childId}) {
    return StreamBuilder<DocumentSnapshot>(
      stream: FirebaseFirestore.instance
          .collection('kis') // Access the 'kids' collection
      ...
      builder: (context, snapshot) {
        ...
        final spendingAmount =
            jarData['spendings']?['amount']?.toString() ?? '0';
...
//notifications data
  ...
  Widget coinTempData({required String childId}) {
    return Expanded(
      child: StreamBuilder<QuerySnapshot>(
        stream: FirebaseFirestore.instance
            .collection('notifications')

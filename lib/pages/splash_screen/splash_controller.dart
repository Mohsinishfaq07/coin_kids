...
class SplashController extends GetxController {
  ...
  void _checkLoginStatus() async {
    ...
    if (user != null) {
      ...
      if (firebaseAuthController.email.isNotEmpty && firebaseAuthController.pin.isNotEmpty) {
        try {
          ...
          final isParent = await _checkIfParent(firebaseAuthController.email.value);
          if (isParent) {
            Get.off(() => ParentBottomNavigationBar());
          } else {
            Get.off(() => KidBottomNavScreen());
  ...
  Future<bool> _checkIfParent(String email) async {
    try {
      final parentSnapshot = await FirebaseFirestore.instance.collection('parents').where('email', isEqualTo: email).get();
      ...
      final kidSnapshot = await FirebaseFirestore.instance.collection('kids').where('email', isEqualTo: email).get();
